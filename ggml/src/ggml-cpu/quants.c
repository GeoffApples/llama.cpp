...
void quantize_row_q3_hifi_f32_raw(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % Q3_HIFI_F32_BLOCK_SIZE == 0);
    block_q3_hifi_f32_raw * GGML_RESTRICT y = vy;
    quantize_row_q3_hifi_f32_raw_ref(x, y, k);
}
...
void ggml_vec_dot_q3_hifi_f32_raw_q8_K_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    const block_q3_hifi_f32_raw * GGML_RESTRICT x = vx;
    const block_q8_K * GGML_RESTRICT y = vy;
    const int nb = n / Q3_HIFI_F32_BLOCK_SIZE;
    ...
    for (int i = 0; i < nb; ++i) {
        const block_q3_hifi_f32_raw * xb = &x[i];
        const block_q8_K * yb = &y[i];
...
// Note: ggml_vec_dot_q3_hifi_f32_raw_q8_K uses generic implementation for now (no arch-specific optimizations)
void ggml_vec_dot_q3_hifi_f32_raw_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ggml_vec_dot_q3_hifi_f32_raw_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
}
