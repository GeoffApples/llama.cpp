...
void quantize_row_q3_hifi_ref(const float * GGML_RESTRICT x, block_q3_hifi * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        int outlier_indices[Q3_HIFI_OUTLIERS];
        for (int k_idx = 0; k_idx < Q3_HIFI_OUTLIERS; ++k_idx) {
            int argmax = 0;
            float max_val = mag[0];
            for (int i = 1; i < Q3_HIFI_BLOCK_SIZE; ++i) {
                if (mag[i] > max_val) {
                    max_val = mag[i];
                    argmax = i;
                }
        ...
        quantize_row_q3_K_ref(tmp, &q3k_block, Q3_HIFI_BLOCK_SIZE);
...
static void quantize_row_q3_hifi_impl(const float * GGML_RESTRICT x, block_q3_hifi * GGML_RESTRICT y, int64_t k, const float * GGML_RESTRICT quant_weights) {
    assert(k % Q3_HIFI_BLOCK_SIZE == 0);
    const int64_t nb = k / Q3_HIFI_BLOCK_SIZE;
...
void dequantize_row_q3_hifi(const block_q3_hifi * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        ...
        // The first 110 bytes of block_q3_hifi match Q3_K exactly
        // Since we pass k=256, Q3_K will only process 1 block (nb=1, using x[0])
        dequantize_row_q3_K((const block_q3_K *)block, yb, Q3_HIFI_BLOCK_SIZE);
...
size_t quantize_q3_hifi(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    const size_t row_size = ggml_row_size(GGML_TYPE_Q3_HIFI, n_per_row);
    if (!quant_weights) {
        quantize_row_q3_hifi_ref(src, dst, nrow * n_per_row);
    } else {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q3_hifi_impl(src, (block_q3_hifi*)qrow, n_per_row, quant_weights);
            src += n_per_row;
            qrow += row_size;
        }
    }
...
// ====================== Q4_HIFI: Q4_K layout + adaptive FP16 outliers ======================
// Uses Q4_K's optimized kernels for ~98% of Q4_K speed with better quality
// Outlier count is adaptive: 8 (≤3B), 10 (3-30B), 12 (30-70B), 16 (>70B)
// Massive layers (lm_head, token_embd) use 2× outliers

// Global outlier count for Q4_HIFI quantization (default: 16 for large models)
static int g_q4_hifi_outlier_count = 16;

void ggml_q4_hifi_set_outlier_count(int count) {
    if (count < 8) count = 8;
    if (count > Q4_HIFI_MAX_OUTLIERS) count = Q4_HIFI_MAX_OUTLIERS;
    g_q4_hifi_outlier_count = count;
}

int ggml_q4_hifi_get_outlier_count(void) {
    return g_q4_hifi_outlier_count;
}

void quantize_row_q4_hifi_ref(const float * GGML_RESTRICT x, block_q4_hifi * GGML_RESTRICT y, int64_t k, int outlier_count) {
    assert(k % Q4_HIFI_BLOCK_SIZE == 0);
    const int64_t nb = k / Q4_HIFI_BLOCK_SIZE;
    
    // Clamp outlier count
    if (outlier_count < 8) outlier_count = 8;
    if (outlier_count > Q4_HIFI_MAX_OUTLIERS) outlier_count = Q4_HIFI_MAX_OUTLIERS;

    for (int64_t ib = 0; ib < nb; ++ib) {
        const float * xb = x + ib * Q4_HIFI_BLOCK_SIZE;
        block_q4_hifi * block = &y[ib];

        // Step 1: Find top-N outliers by magnitude
        float mag[Q4_HIFI_BLOCK_SIZE];
        for (int i = 0; i < Q4_HIFI_BLOCK_SIZE; ++i) {
            mag[i] = fabsf(xb[i]);
        }

        int outlier_indices[Q4_HIFI_MAX_OUTLIERS];
        for (int k_idx = 0; k_idx < outlier_count; ++k_idx) {
            int argmax = 0;
            float max_val = mag[0];
            for (int i = 1; i < Q4_HIFI_BLOCK_SIZE; ++i) {
                if (mag[i] > max_val) {
                    max_val = mag[i];
                    argmax = i;
                }
        ...
        // Step 2: Create temporary array with outliers zeroed (pre-zero for faster vec_dot)
        float tmp[Q4_HIFI_BLOCK_SIZE];
        memcpy(tmp, xb, sizeof(tmp));
        for (int k_idx = 0; k_idx < outlier_count; ++k_idx) {
            tmp[outlier_indices[k_idx]] = 0.0f;
        }

        // Step 3: Quantize bulk using Q4_K algorithm (produces Q4_K-compatible layout)
        block_q4_K q4k_block;
        quantize_row_q4_K_ref(tmp, &q4k_block, Q4_HIFI_BLOCK_SIZE);

        // Step 4: Copy Q4_K fields to our block (first 144 bytes are identical layout)
        block->d = q4k_block.d;
        block->dmin = q4k_block.dmin;
        memcpy(block->scales, q4k_block.scales, sizeof(block->scales));
        memcpy(block->qs, q4k_block.qs, sizeof(block->qs));

        // Step 5: Store outlier count and outliers (indices and FP16 values)
...
static void quantize_row_q4_hifi_impl(const float * GGML_RESTRICT x, block_q4_hifi * GGML_RESTRICT y, int64_t k, const float * GGML_RESTRICT quant_weights, int outlier_count) {
    assert(k % Q4_HIFI_BLOCK_SIZE == 0);
    const int64_t nb = k / Q4_HIFI_BLOCK_SIZE;
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float * xb = x + ib * Q4_HIFI_BLOCK_SIZE;
        const float * qw = quant_weights ? quant_weights + ib * Q4_HIFI_BLOCK_SIZE : NULL;
        block_q4_hifi * block = &y[ib];

        // Step 1: Find top-N outliers by weighted magnitude (imatrix-aware)
        float mag[Q4_HIFI_BLOCK_SIZE];
        for (int i = 0; i < Q4_HIFI_BLOCK_SIZE; ++i) {
            mag[i] = fabsf(xb[i]) * (qw ? qw[i] : 1.0f);
        }
        ...
        // Step 3: Quantize bulk using Q4_K algorithm
        block_q4_K q4k_block;
        quantize_row_q4_K_ref(tmp, &q4k_block, Q4_HIFI_BLOCK_SIZE);

        // Step 4: Copy Q4_K fields to our block
        block->d = q4k_block.d;
        block->dmin = q4k_block.dmin;
        memcpy(block->scales, q4k_block.scales, sizeof(block->scales));
        memcpy(block->qs, q4k_block.qs, sizeof(block->qs));
...
void dequantize_row_q4_hifi(const block_q4_hifi * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % Q4_HIFI_BLOCK_SIZE == 0);
    const int64_t nb = k / Q4_HIFI_BLOCK_SIZE;

    for (int64_t ib = 0; ib < nb; ++ib) {
        const block_q4_hifi * block = &x[ib];
        float * yb = y + ib * Q4_HIFI_BLOCK_SIZE;

        // Dequantize using Q4_K algorithm for single block
        // The first 144 bytes of block_q4_hifi match Q4_K exactly
        dequantize_row_q4_K((const block_q4_K *)block, yb, Q4_HIFI_BLOCK_SIZE);

        // Overwrite outlier positions with FP16 values
        const int outlier_count = block->outlier_count;
        for (int k_idx = 0; k_idx < outlier_count && k_idx < Q4_HIFI_MAX_OUTLIERS; ++k_idx) {
            const int idx = block->outlier_idx[k_idx];
            if (idx < Q4_HIFI_BLOCK_SIZE) {
                yb[idx] = GGML_FP16_TO_FP32(block->outlier_vals[k_idx]);
            }
        }
    }
...
size_t quantize_q4_hifi(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    const size_t row_size = ggml_row_size(GGML_TYPE_Q4_HIFI, n_per_row);
    const int outlier_count = g_q4_hifi_outlier_count;
    
    if (!quant_weights) {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q4_hifi_ref(src, (block_q4_hifi*)qrow, n_per_row, outlier_count);
            src += n_per_row;
            qrow += row_size;
        }
    } else {
        char * qrow = (char *)dst;
        for (int64_t row = 0; row < nrow; ++row) {
            quantize_row_q4_hifi_impl(src, (block_q4_hifi*)qrow, n_per_row, quant_weights, outlier_count);
            src += n_per_row;
            qrow += row_size;
        }
    }
...
// ====================== Q4_HIFI_RESIDUAL: Revolutionary INT8 Residual Quantization ======================
// Instead of storing raw FP16 outlier values, store residuals (weight - Q4_K_approx) in INT8
// Benefits:
//   - 50% smaller outlier storage (2 bytes vs 3 bytes per outlier)
//   - Better reconstruction accuracy (residuals are tiny, INT8 is sufficient)
//   - Q4_K base = Q4_K_M speed, INT8 residuals add minimal overhead
//   - Result: Smaller than Q4_K_M with better quality

void quantize_row_q4_hifi_residual_ref(const float * GGML_RESTRICT x, block_q4_hifi_residual * GGML_RESTRICT y, int64_t k, const float * GGML_RESTRICT imatrix) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    const int max_outliers = Q4_HIFI_RESIDUAL_MAX_OUTLIERS;
    
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float * xb = x + ib * QK_K;
        const float * im = imatrix ? (imatrix + ib * QK_K) : NULL;
        block_q4_hifi_residual * block = &y[ib];
        
        // Step 1: Find top-8 outliers by weighted magnitude (imatrix-aware)
        float mag[QK_K];
        for (int i = 0; i < QK_K; ++i) {
            float weight = im ? im[i] : 1.0f;
            mag[i] = fabsf(xb[i]) * weight;
        }
        
        // Find top-8 outlier indices
        int outlier_idx[Q4_HIFI_RESIDUAL_MAX_OUTLIERS];
        for (int k_idx = 0; k_idx < max_outliers; ++k_idx) {
            ...
            outlier_idx[k_idx] = max_idx;
            mag[max_idx] = -1.0f;  // Mark as used
        }
        
        // Step 2: Zero outliers and quantize bulk as Q4_K
        float tmp[QK_K];
        ...
        // Quantize bulk using Q4_K algorithm
        quantize_row_q4_K_ref(tmp, (block_q4_K *)block, QK_K);
        
        // Step 3: Dequantize Q4_K to get approximation, then compute residuals
        float approx[QK_K];
        dequantize_row_q4_K((const block_q4_K *)block, approx, QK_K);
        
        // Compute residuals and find max for scaling
        float residuals[Q4_HIFI_RESIDUAL_MAX_OUTLIERS];
        float max_residual = 0.0f;
        for (int k_idx = 0; k_idx < max_outliers; ++k_idx) {
            int idx = outlier_idx[k_idx];
            residuals[k_idx] = xb[idx] - approx[idx];  // True value minus Q4_K approx
        ...
        if (max_residual < 1e-8f) {
            max_residual = 1e-8f;
        }
        
        // Step 4: Store outlier data
        block->outlier_count = max_outliers;
        block->residual_scale = max_residual;
        
        for (int k_idx = 0; k_idx < max_outliers; ++k_idx) {
            block->outlier_idx[k_idx] = (uint8_t)outlier_idx[k_idx];
            // Quantize residual to INT8 [-127, 127]
...
void dequantize_row_q4_hifi_residual(const block_q4_hifi_residual * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    
    for (int64_t ib = 0; ib < nb; ++ib) {
        const block_q4_hifi_residual * block = &x[ib];
        float * yb = y + ib * QK_K;
        
        // Step 1: Dequantize Q4_K base (first 144 bytes are compatible)
        dequantize_row_q4_K((const block_q4_K *)block, yb, QK_K);
        
        // Step 2: Add residual corrections
        const int outlier_count = block->outlier_count;
        const float scale = block->residual_scale;
        
        for (int k_idx = 0; k_idx < outlier_count && k_idx < Q4_HIFI_RESIDUAL_MAX_OUTLIERS; ++k_idx) {
            const int idx = block->outlier_idx[k_idx];
            if (idx < QK_K) {
                // Reconstruct: Q4_K_approx + (INT8_residual / 127) * scale
                float residual = (block->residual_vals[k_idx] / 127.0f) * scale;
                yb[idx] += residual;
            }
        }
    }
...
size_t quantize_q4_hifi_residual(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * imatrix) {
    const size_t row_size = ggml_row_size(GGML_TYPE_Q4_HIFI_RESIDUAL, n_per_row);
    
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        const float * im = imatrix ? (imatrix + row * n_per_row) : NULL;
        quantize_row_q4_hifi_residual_ref(src, (block_q4_hifi_residual*)qrow, n_per_row, im);
        src += n_per_row;
        qrow += row_size;
    }
...
// Q3_HIFI: 3-bit + FP16 outliers per 256 weights
// Q3_HIFI_BLOCK_SIZE and Q3_HIFI_OUTLIERS are defined in ggml.h
